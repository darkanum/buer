...
| D6 | **Rascunho não revisado sai `confidence: 'medium'`, e não `'low'` como a spec escreve** | Seguir a §5.3/§10 ao pé da letra e emitir `'low'` para todo rascunho | Desvio consciente da spec, registrado aqui para não ser tácito. A §5.3 e a §10 escreveram `'low'` supondo um rascunho de fonte única; aqui a confiança é **derivada de concordância real entre três fontes independentes** — `'medium'` só sai quando algum campo é sustentado por 2+ fontes e nenhum campo escalar está em contradição; caso contrário sai `'low'` de qualquer jeito. Isso é mais evidência do que a spec contemplava, e `authoredBy: 'researched'` continua distinguindo máquina de humano em todo lugar que importa (`validateMeta` proíbe `researched` + `high`; a CLI e o relatório do lote dizem a origem). **Custo de estar errado:** uma ficha ou arquétipo *single-sourced mal detectado* sairia um degrau acima do que merece e um revisor apressado poderia promovê-lo sem ler o `.md` da pesquisa — recuperável, porque a proveniência e as notas de divergência continuam no arquivo, mas real. Se aparecer evidência de que acontece, o conserto é uma linha em `reconcileCharacter` e em `buildArchetypeDrafts`. |
