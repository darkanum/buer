...
// `GET` wrapper that supplies the real R2-backed implementation — same
// shape as app/api/ingest/route.ts's `handleIngest`/`IngestDeps` — so
// test/img.test.ts exercises the real branching logic against injected
...
// at module import time) so importing this route — as test/img.test.ts does,
...
// ---------------------------------------------------------------------------
// Route handler (thin wrapper) — Next.js 16 App Router dynamic segments hand
// `params` in as a Promise (same convention as every other dynamic route
// since Next 15).
// ---------------------------------------------------------------------------
